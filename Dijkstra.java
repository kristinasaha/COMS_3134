...
UNI ks3401
Data Structures 
Dijkstra class - template was given to us */
...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        // TODO
	  return Math.sqrt(Math.pow((ux - vx), 2) + Math.pow((uy - vy), 2)); // Replace this
  }
  ...
  public void doDijkstra(String s) {
	...
	// Decrease s's distance to 0
			bh.decreaseKey(bh.find(getVertex(s)), 0);
			System.out.println(s); 
			
			while(!bh.isEmpty())
			{
				Vertex v = bh.deleteMin();	// Delete vertex with min d_v
				v.known = true;

				// Iterate through all the adjacent edges...
				for(Edge e : v.adjacentEdges)
				{
					// Check neighbor vertex d_w > d_v + c_v,w.
					Vertex w = e.target;
					if(!w.known)
					{
						...
						{
							//Update neighbor's vertex's d_v value and prev
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    doDijkstra(s);
    // TODO
    doDijkstra(s);	// Performs Dijkstra's algorithm
	List<Edge> path = new LinkedList<>();	// New LL to store path
	return getPath(getVertex(t), path);		// Call helper method
}
  private List<Edge> getPath(Vertex w, List<Edge> path)
	{
		// If null, reached base case.
		
		if(w.prev != null)
		{
			// Get the path up to the current vertex. Then add edge from
			// previous vertex to this vertex. Return final path
